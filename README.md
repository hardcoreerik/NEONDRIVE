...
==[EOF]==
## CYD Port Status (as of this zip)
This project is a *CYD bring-up + port scaffold* for the original Porkchop firmware.

### What was fixed
- Corrected CYD display config to **ILI9341_2_DRIVER** (common for ESP32-2432S024 clone panels)
- Set **landscape rotation** and forced **invertDisplay(true)** to eliminate clipping/bottom-noise artifacts on many boards
- Explicit **MISO pin** + `SPI.begin(sck,miso,mosi)` to avoid weird SPI read/tearing behavior
- Backlight confirmed on **GPIO21**

### What you should see
- Screen fully clears (no leftover boot image)
- Large centered **EAT THE RICH** text in landscape
- Bottom status line
- Touch taps print **raw coordinates** to Serial (115200) and paint small yellow dots (approx mapped)

### Next steps (porting the real Porkchop firmware)
The original Porkchop firmware source is preserved under `./M5PORKCHOP/`.
Next we build a CYD hardware abstraction layer (HAL) to replace M5Cardputer display+keyboard calls with:
- TFT_eSPI drawing
- Touch-based UI input (soft buttons / gestures)
- Optional BLE keyboard later
