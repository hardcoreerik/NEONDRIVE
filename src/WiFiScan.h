// Legacy WiFiScan interface and scan-mode definitions.
#pragma once
#include <WiFi.h>
#include "main.h" // for ApRecord and constants

// Export AP scan storage for modules that consume scan results.
...
#define WIFI_HOSTSPOT 69 // Reserved hotspot mode
...
{
  private:
    // Wardriver implementation inspired by https://github.com/JosephHewitt
    int arp_count = 0;
    #ifndef HAS_PSRAM
    ...
    float y_pos_x_old = 120; // previous graph Y-axis position of X value
    float y_pos_y; // current graph Y-axis position of Y value
    float y_pos_y_old = 120; // previous graph Y-axis position of Y value
    float y_pos_z; // current graph Y-axis position of Z value
    float y_pos_z_old = 120; // previous graph Y-axis position of Z value
    int midway = 0;
    byte x_scale = 1; // graph X-axis scale, controlled by touchscreen buttons
    byte y_scale = 1;
    ...
    // Static SSID list used by the funny beacon spam mode.
    ...
    // Security conditions used for Pineapple detection.
    ...
    // Table of suspicious vendors for Pineapple detection
    static const SuspiciousVendor suspicious_vendors[];
    static const int NUM_SUSPICIOUS_VENDORS;

    // Tracks whether AP list limit has already been reported (initialized in RunSetup).
    bool ap_list_full_reported;

    // MultiSSID tracking structures
    ...
    // Minimal beacon template packet
